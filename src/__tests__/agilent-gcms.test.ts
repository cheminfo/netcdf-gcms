...
import { fromAgilentGCMS, netcdfGcms } from '../index.ts';

const pathFiles = join(import.meta.dirname, 'data');

test('Agilent format - Agilent file', () => {
  const data = readFileSync(join(pathFiles, 'agilent-gcms.cdf')).buffer;
...
test('Agilent format - fromAgilent', () => {
  const data = readFileSync(join(pathFiles, 'agilent-gcms.cdf')).buffer;
