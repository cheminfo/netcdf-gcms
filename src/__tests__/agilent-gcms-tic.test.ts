...
test('Agilent HPLC/MS with only HPLC - Agilent file', () => {
  const data = readFileSync(join(pathFiles, 'agilent-gcms-tic.cdf')).buffer;
  const json = netcdfGcms(data);

  expect(json.times).toHaveLength(1645);

  for (const seriesItem of json.series) {
    expect(seriesItem.data).toHaveLength(1645);
  }
});

test('Agilent HPLC/MS with only HPLC - fromAgilent', () => {
  const data = readFileSync(join(pathFiles, 'agilent-gcms-tic.cdf')).buffer;
...
test('Agilent HPLC/MS with only HPLC - with meta', () => {
  const data = readFileSync(join(pathFiles, 'agilent-gcms-tic.cdf')).buffer;
  const json = netcdfGcms(data);
